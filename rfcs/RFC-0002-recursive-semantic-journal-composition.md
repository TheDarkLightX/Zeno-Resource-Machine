...
**Created:** 2026-07-12  
**Target version:** ZRM/ZRPF 0.2 draft  
**Change class:** E

## Summary

This RFC defines the semantic boundary between ZRM and the Zeno Recursive Proof
Fabric (ZRPF). It makes an accepted ZRM journal the native postcommit semantic
...
- no duplicated transition, nullifier, resource creation, reward, or message;
- complete accounting and transformation composition;
- carry-state and cross-partition message continuity;
...
- Support a deliberately simple bounded reference profile before optimized
  accumulators.
- Preserve proof-system neutrality in ZRM while allowing RISC0 and other
  recursive backends in ZRPF adapters.
...
**Semantic summary:** A canonical commitment to the segment endpoints,
ordered manifest, descendant identity sets, accounting/effects, messages,
carry state, and DA coverage.
...
A postcommit ZRPF adapter accepts only a sealed or independently authenticated
`AcceptedJournal` source. Journal bytes alone are data.
...
- resource, nullifier, claim, accounting, evidence, provenance, DA, and complete
  commit-effects commitments required by the active journal version;
...
Addition is checked. `leaf_count` is nonzero and bounded.
...
`position` is dense and canonical. The manifest MUST reject omission,
...
SemanticSegmentSummaryV1 {
  ...
  commit_effects_accumulator_root
  commit_effect_count
...
separate profile and proof format.
...
- Accounting and effects are sorted unique rows by their typed dimensions or
  effect keys.
- Message order is profile-defined; a message key cannot silently be treated as
  a set when delivery order is semantic.
...
accounting/effects = journal committed rows/effects
messages/carry/DA = journal profile commitments
```
...
The version relation required by the active journal profile must also hold for
each leaf. The first serialized profile uses one globally serialized machine
state and therefore expects consecutive versions.
...
### 7.7 Complete effect composition

The commit-effects accumulator combines every child effect exactly once. It
must preserve effect keys and reject duplicate one-shot keys.
...
### 7.8 Message cancellation

When the profile permits cross-partition messages, each internal message must
have exactly one matching outbox and inbox record under a canonical key and
payload commitment.

For adjacent segments:

```text
internal = left.outbox intersect right.inbox
```

Matching internal messages cancel from the parent's external boundary only
after exact key, payload, source, destination, ordering, and replay-domain
checks. Unmatched messages remain in the parent inbox or outbox as defined by
the profile. Duplicate consumption or creation rejects.

A profile without messages uses named domain-separated empty roots.

### 7.9 Carry continuity

```text
left.carry_post_root == right.carry_pre_root
```

The parent carries `left.carry_pre_root` to `right.carry_post_root`. Carry roots
cannot stand in for machine-state roots, accounting, or messages unless the
profile defines and proves that relation.

### 7.10 Data availability
...
parent accounting/effects = exact checked combinations
parent external messages = profile-defined boundary composition
parent.carry_pre = left.carry_pre
parent.carry_post = right.carry_post
...
SemanticSegmentHash = H_semantic_segment_v1(
  ...
  accounting and effect roots and counts,
  message boundary roots and counts,
  carry roots,
...
- sorted accounting and effect rows;
- checked `u128` intermediate arithmetic;
- explicit message and carry vectors where enabled;
...
| Message created/consumed twice | exact inbox/outbox key matching | message algebra model |
| Carry state substituted | exact carry continuity | mismatch mutants |
...
## 15. ESSO bounded oracle

`zrpf_semantic_composition_v1` models three ordered leaf summaries with:

- bounded state endpoints;
- checked bounded leaf counts;
- bounded effect totals;
- two descendant-membership bits;
- pairwise disjointness;
- left-associated and right-associated folds.

It requires both groupings to produce equal endpoints, counts, effects, and
member union. A targeted mutant omits the middle leaf count from the
right-associated fold and must produce an invariant counterexample.

This model does not represent cryptographic roots, arbitrary set sizes,
messages, carry, DA, or proof systems. It is a small algebraic design oracle,
...
ComposeEffectsExact
ComposeMessagesExactBoundary
ComposeCarryContinuous
ComposeDaCoverage
ComposeAssociative
...
summaries under one fixed semantic profile. ESSO supplies only bounded examples.
...
- RFC-0001's future accepted-journal version must expose complete effects before
  RFC-0002 can claim complete semantic composition.
...
- bounded ESSO composition and omission mutant;
...
- message and carry mismatch models;
...
ESSO remains development-only. ZRM core does not depend on a recursive SDK.
ZRPF adapters may depend on governed proof-system libraries under explicit TCB,
release, and provenance policies.
...
- exact bounded descendant, accounting, effect, message, carry, and DA
  composition for the named profile;
