...
use zrm_policy::{
    ...
    VerifierCostModelCandidateV1, VerifierCostModelV1, VerifierCostQuoteV1,
...
#[test]
fn machine_policy_accepts_local_and_required_admission_exactly() -> Result<(), ZeroValueError> {
    let local = MachinePolicyV1::try_from(machine_candidate(AdmissionModeV1::LocalKernel, None)?);
    assert!(local.is_ok());
    if let Ok(policy) = local {
    ...
    let required = MachinePolicyV1::try_from(machine_candidate(
        AdmissionModeV1::RequiredVerifier,
        Some(verifier_id),
    )?);
    assert!(required.is_ok());
    if let Ok(policy) = required {
        assert_eq!(
...
#[test]
fn every_machine_limit_accepts_its_ceiling_and_rejects_ceiling_plus_one() {
    let ceiling = PolicyLimitsV1::protocol_ceiling().as_candidate();
    assert!(PolicyLimitsV1::try_from(ceiling).is_ok());
    ...
    assert_eq!(
        ...
        Err(PolicyValidationErrorV1::LimitExceedsProtocolCeiling {
            ...
            actual: u64::from(PolicyLimitsV1::MAX_STORAGE_WRITE_BYTES) + 1,
...
#[test]
fn resource_kind_policy_enforces_unit_quantity_and_lifecycle_bounds() -> Result<(), ZeroValueError>
{
    ...
    let mut lifecycle = resource_kind_candidate()?;
    ...
    assert!(lifecycle_policy.is_ok());
    if let Ok(lifecycle_policy) = lifecycle_policy {
        assert_eq!(
...
#[test]
fn validated_policy_getters_preserve_typed_candidates() -> Result<(), ZeroValueError> {
...
#[test]
fn verifier_and_context_getters_preserve_typed_candidates() -> Result<(), ZeroValueError> {
...
#[test]
fn policy_derived_cost_quotes_preserve_formula_and_reservation() -> Result<(), ZeroValueError> {
    let mut candidate = verifier_candidate()?;
...
#[test]
fn policy_derived_cost_quotes_preserve_every_hidden_bound() -> Result<(), ZeroValueError> {
    let mut candidate = verifier_candidate()?;
    candidate.max_artifact_bytes = 10;
    candidate.max_public_input_bytes = 20;
    candidate.max_public_output_bytes = 30;
    ...
    assert!(capped_policy.is_ok());
    let Ok(capped_policy) = capped_policy else {
        return Ok(());
    };
    assert_eq!(
...
-> Result<(), ZeroValueError> {
    let machine = MachinePolicyV1::try_from(machine_candidate(AdmissionModeV1::LocalKernel, None)?);
    let verifier = VerifierPolicyV1::try_from(verifier_candidate()?);
    assert!(machine.is_ok());
    assert!(verifier.is_ok());
    if let (Ok(machine), Ok(verifier)) = (machine, verifier) {
        assert_eq!(
...
#[test]
fn admission_verifier_candidate_requires_mode_and_exact_policy_id() -> Result<(), ZeroValueError> {
    let verifier = VerifierPolicyV1::try_from(verifier_candidate()?);
...
#[test]
fn verifier_compatibility_rejects_every_mismatch_before_authority() -> Result<(), ZeroValueError> {
    let machine = MachinePolicyV1::try_from(machine_candidate(AdmissionModeV1::LocalKernel, None)?);
    assert!(machine.is_ok());
    ...
    if let Ok(verifier) = VerifierPolicyV1::try_from(candidate) {
        assert_eq!(
            machine.validate_verifier_candidate_compatibility(&verifier, 15),
...
#[test]
fn verifier_compatibility_accepts_exact_machine_caps_and_rejects_one_more()
-> Result<(), ZeroValueError> {
    let machine = MachinePolicyV1::try_from(machine_candidate(AdmissionModeV1::LocalKernel, None)?);
    let Ok(machine) = machine else {
        return Ok(());
    ...
    if let Ok(verifier) = VerifierPolicyV1::try_from(exact) {
        assert_eq!(
            machine.validate_verifier_candidate_compatibility(&verifier, 15),
            Ok(())
        );
    ...
    if let Ok(verifier) = VerifierPolicyV1::try_from(artifact_over) {
        assert_eq!(
            machine.validate_verifier_candidate_compatibility(&verifier, 15),
            Err(VerifierCompatibilityErrorV1::ArtifactLimitExceedsMachine)
        );
    ...
    if let Ok(verifier) = VerifierPolicyV1::try_from(cost_over) {
        assert_eq!(
            machine.validate_verifier_candidate_compatibility(&verifier, 15),
            Err(VerifierCompatibilityErrorV1::VerifierCostLimitExceedsMachine)
        );
    ...
    let Ok(total_bound_machine) = total_bound_machine else {
        return Ok(());
    };

    let mut total_exact = verifier_candidate()?;
    total_exact.max_artifact_bytes = 600_000;
    if let Ok(verifier) = VerifierPolicyV1::try_from(total_exact) {
        assert_eq!(
