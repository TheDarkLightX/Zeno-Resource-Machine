...
Native/build surface: `libfuzzer-sys` uses a build script and `cc` to build and link LLVM libFuzzer support. `cc`, `find-msvc-tools`, `jobserver`, and `shlex` participate in native build orchestration; `getrandom`, `libc`, and `r-efi` add platform-specific tooling paths. This surface is accepted only in the isolated offline/CI fuzz workspace.
