...
| ZRM-05 | Diagnostic privacy | Opaque values and raw wire candidates printed complete bytes, including nonce | All opaque 32-byte values use constant redaction; `ResourceWireV1` manually redacts fixed-width candidates; nested intrinsic formatting is nonce-value-independent | Future artifacts, witnesses, private state, errors, log sinks, and evidence bundles need separate disclosure review and zeroization where applicable |
